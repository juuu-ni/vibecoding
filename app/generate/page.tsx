      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <section className="bg-white p-6 md:p-10 shadow-[0_1px_3px_rgba(0,0,0,0.05)] rounded-sm border border-slate-50">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {previews.map((src, i) => (
                ...
                >
                  <img src={src} className="w-full h-full object-cover rounded-md border border-slate-100 shadow-sm transition-transform duration-300 group-active:scale-95 group-active:opacity-50" alt="preview" />
                  
                  <div className="absolute top-2 left-2 flex items-center gap-1.5">
                    <div className="bg-black/50 backdrop-blur-sm text-white text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1">
                  ...
                  <button 
                    ...
                    className="absolute -top-2 -right-2 bg-white text-slate-500 p-1.5 rounded-full shadow-md border border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity hover:text-red-500"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
            ...
            <div className="mt-6 flex items-start gap-2 text-slate-400 text-[12px] bg-slate-50 p-4 rounded-md">
              ...
              <p>사진을 드래그하여 배치한 순서대로 블로그 본문이 구성됩니다. 핵심 이미지를 앞쪽에 배치해보세요!</p>
            </div>
          </section>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  ...
                  <input name="storeName" required className="w-full bg-slate-50 border-none pl-12 pr-4 py-3.5 rounded-md font-semibold outline-none focus:ring-1 focus:ring-blue-500 transition-all" placeholder="매장 이름" />
                </div>
                <div className="relative">
                  <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input name="locationText" required className="w-full bg-slate-50 border-none pl-12 pr-4 py-3.5 rounded-md font-semibold outline-none focus:ring-1 focus:ring-blue-500 transition-all" placeholder="지역 (예: 성수동)" />
                </div>
              </div>
              <div className="relative">
                ...
                <textarea 
                  name="userNotes" 
                  className="w-full h-40 bg-slate-50 border-none rounded-md pl-12 pr-4 py-4 text-slate-700 placeholder:text-slate-400 resize-none outline-none focus:ring-1 focus:ring-blue-500 transition-all font-medium" 
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {TONE_STYLES.map(t => (
                  <button
                    ...
                    className={cn(
                      "w-full px-4 py-3 rounded-md text-left text-xs font-bold flex items-center justify-between border transition-all",
                      selectedTone === t 
                  ...
                  >
                    {t}
                    {selectedTone === t && <Check className="w-3.5 h-3.5" />}
                  </button>
                ))}
              </div>
              ...
              {selectedTone === "나만의 문체(샘플 입력)" && (
                <div className="pt-2 animate-in fade-in slide-in-from-top-2 duration-300">
                  <label className="text-[10px] font-bold text-blue-600 mb-1.5 block">이전 블로그 글 샘플 (300자 이상 권장)</label>
                  <textarea 
                    value={customToneSample}
                    onChange={(e) => setCustomToneSample(e.target.value)}
                    className="w-full h-32 bg-blue-50/30 border border-blue-100 rounded-md p-3 text-[11px] focus:ring-1 focus:ring-blue-500 outline-none"
                    placeholder="본인이 직접 쓴 블로그 글을 복사해서 붙여넣어 주세요. AI가 말투와 서술 방식, 이모지 사용 습관 등을 분석합니다."
                  />
                </div>
              )}
            </div>
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full bg-slate-900 text-white py-5 rounded-md font-bold text-lg hover:bg-slate-800 transition-all disabled:bg-slate-200 shadow-xl shadow-slate-200 flex flex-col items-center justify-center gap-1 overflow-hidden group"
          >
            {loading ? (
              <>
